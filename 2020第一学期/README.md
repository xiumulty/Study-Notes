...
# 2021年2月11日 #
- 大年30，准备休息一天的，但是学长给了一个红包题，又爬起来肝了一下，基本思路已经确定，但是要写出来还是需要一点时间的。

# 2021年2月10日 #
- 感冒还没好，希望早点好哇，太难受了
- 把21-28爆破题写完了，wp也写完了，学到挺多的
- 装了一下kali，在kali里面使用了一波php_mt_seed，用来破解伪加密的小工具，kali可以运行php脚本，打开终端使用命令`php xxx.php`就可以，kali是一个宝藏啊，之前一直没去装，里面很多工具可惜不会用
- 重新装了一个新版本的burp，2020.1的版本，不知道为什么12月的运行不起来，百度了也没找到结果，不过1月份的也够用了，还去网上找了一个汉化包，舒服多了
- 把sqlilabs和uploadlabs配置好了，有时间把它们写了
- 遇到一些题发现涉及到jsp，不知道什么时候开始学，感觉php还没学扎实

# 2021年2月9日 #
- 仍然感冒中，很累
- 做了一下ssrf的笔记，网课里讲的ssrf主要是利用`file_get_contents()`这类的函数来向服务器请求一些信息，由于是借助本地已经有的函数来请求的，所以ip是本地的ip，这样就是一种身份的伪造
- 把之前学长给的题目写了，看了sql的网课，做起来还是不难，由于引号被过滤，所以用\来注释掉引号，再利用下一对引号的第一个来闭合前一对引号
- 想去复现杭电的题，看了wp也是一脸懵逼，主要是太多东西没学，有点不知所措，什么http走私之类的，慢慢看下去吧
- starsnow的题写了三个签到题，web的是一个都没写出来，太菜了

# 2021年2月8日 #
- 安装了一个新版的burp
- 网课讲了sql注入，一开始还是听得懂的，但是从盲注开始就听不怎么懂了，到时候拿到课件再看看，整理一波笔记到博客

# 2021年2月7日 #
- 下午改了一下注册登录界面，使用`addslashes()`函数实现了对sql注入的防御，这个函数是一个转义函数，会在引号前加上反斜杠\，转义后引号就无法实现它封闭的效果了，进而达到防止sql注入的效果
- 晚上看了直播，讲了ssrf，具体信息拿到课件做在博客里
- 回顾昨天晚上的一句话木马，记录昨天的笔记写在博客里

# 2021年2月6日 #
- 还是在感冒，睡了一天。
- 晚上看了一波web的网课，主要学习了一句话木马，一句话木马涉及的东西挺多的，主要是通过回调函数来做坏事，比如`eval()`函数就是识别并执行里面的字符串，可以是`phpinfo()`。。。但是一般这种会被屏蔽掉，所以可以考虑可变函数来操作，`eval()`是不支持可变函数的调用的，但是其他的可以，比如`asset()`具体遇到再写进博客里
- 写了一个爆破题，这个爆破题涉及到了编码和拼接，可以在自定义里面自己设计自己的爆破格式，wp写在了博客里
- 下午看了一下自己的博客，发现崩了，重新去dns配置了一下，直接挂到github里了，放弃了coding,反正都慢

# 2021年2月5日 #
- 又感冒了，小年休息了一天，晚上刷了一道web犯困去睡觉了

# 2021年2月4日 #
- 同学聚会玩了一天。。。
- 晚上稍微看了两个难题，没搞懂，明天开始先暂停刷题，先打一波基础，最近有点眼高手低
- 安装了一个sqlmap，不过还不知道用，现在暂时也用不上，先放着吧

# 2021年2月2-3日 #
- 刷题中，写了一波杭电的题，太菜了，只写出了一题，唉，菜是原罪
- 写了一些web的基础题，了解了一些之前没注意到的小细节
- 本来是想学一下序列化和反序列化，只看懂了原理，但是做题的话php类和对象还没学，只靠py的对象基础还不够，准备加强一波php基础
