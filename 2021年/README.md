...
- 有个海南大学的内部赛，混进去玩了一下，主要是这次的杂项，学到了一个usb流量分析，试了两个密码学的题，感觉还行，web方面差一个题，这个题只能晚上12点做。。。是一个flask题，要求提交一个参数，这个参数会进一个函数，但是这个函数是不知道的，唯一能操作的就是这个提交的参数，题目描述说写下出题者的姓名，但是都试过都没有，就没管这种傻批题了
